...
document.querySelector(".crere_par").addEventListener('click', function myFunction() {
  ...
  fetch('http://127.0.0.1:5000/changer_role?code='+code_p+'&roles='+encodeURIComponent(JSON.stringify(dico_roles)))
  .then(response => response.json())
  .then(data => {
    if (data['reponce'] ==="0"){
      document.location.href ="./indentifent.html?code=" + encodeURIComponent(code_p);
  }})
})
